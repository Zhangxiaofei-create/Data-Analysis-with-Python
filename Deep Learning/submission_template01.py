...
model.eval()  # Переводим модель в режим оценки перед сохранением
model_path = "model.pth"

# Проверка, что директория существует
os.makedirs(os.path.dirname(model_path), exist_ok=True)

# Сохранение модели с помощью torch.jit.script
scripted_model = torch.jit.script(model.cpu())  # Переводим модель на CPU перед сохранением
