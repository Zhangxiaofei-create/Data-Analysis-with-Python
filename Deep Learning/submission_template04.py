...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.pool1 = nn.MaxPool2d(kernel_size=2)  # Используем MaxPool2d
        self.conv2 = nn.Conv2d(in_channels=3, out_channels=5, kernel_size=(3, 3))
        self.pool2 = nn.MaxPool2d(kernel_size=2)

        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(in_features=5 * 6 * 6, out_features=100)  # Размеры нужно проверить и откорректировать
...
# pool1
assert model.pool1.kernel_size == (2, 2), "неверный размер ядра у pool1"  # Ожидается (2, 2)
...
# Проверка размера пула через атрибуты
# Проверка для pool1
assert (model.pool1.kernel_size[0] == 2 and model.pool1.kernel_size[1] == 2), "неверный размер ядра у pool1"
